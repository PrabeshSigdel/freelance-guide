...
$media_type = (string) get_theme_mod('aatf_explore_hero_media_type', 'none');
$hero_image = (string) get_theme_mod('aatf_explore_hero_image', '');
$hero_title = (string) get_theme_mod('aatf_explore_hero_title', '');
$hero_text  = (string) get_theme_mod('aatf_explore_hero_text', '');

if ($hero_title === '') {
    $hero_title = __('Explore The Worlds', 'aatf-expedition-base');
}
if ($hero_text === '') {
    $hero_text = __("People Don't Take, Trips Take People", 'aatf-expedition-base');
}
...
<!-- ── Hero ──────────────────────────────────────────────────────── -->
<section class="hero-bg relative min-h-[420px] flex flex-col items-center justify-center text-center"
    style="background-image: url('<?php echo esc_url($bg_url); ?>');">
    <div class="absolute inset-0 bg-black/40"></div>

    <div class="relative z-10 px-6 py-20">
        <h1 class="font-display text-5xl md:text-6xl font-bold text-[var(--brand-orange)] mb-4 drop-shadow-lg tracking-wide">
            <?php echo esc_html($hero_title); ?>
        </h1>
        <p class="text-white text-lg md:text-xl font-light tracking-[0.2em] drop-shadow">
...
<!-- ── Search Bar ────────────────────────────────────────────────── -->
<div class="bg-white mt-5 shadow-[0_8px_40px_rgba(0,0,0,0.13)] mx-auto max-w-7xl rounded-lg relative z-20 flex items-center divide-x divide-gray-200 overflow-hidden border border-gray-100">
    ...
    <!-- Filter icon -->
    <div class="px-4 py-5 cursor-pointer hover:bg-orange-50 transition-colors">
    ...
    <!-- Search Button -->
    <button class="bg-[var(--brand-orange)] rounded-xl hover:bg-gray-900 text-white font-bold px-6 py-[16px] flex items-center gap-2 transition-colors text-sm tracking-wider uppercase whitespace-nowrap">
